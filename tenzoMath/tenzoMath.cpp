...
void TenzoMath::calculatePos(std::array<double, 6>& curPos, std::array<double, 6> ftReadings)
{
    ...
    forces.at<double>(0, 0) = (abs(ftReadings[0]) < threshold ? 0 : ftReadings[0] * coefForces);
    forces.at<double>(0, 1) = (abs(ftReadings[1]) < threshold ? 0 : ftReadings[1] * coefForces);
    forces.at<double>(0, 2) = (abs(ftReadings[2]) < threshold ? 0 : ftReadings[2] * coefForces);
    torques.at<double>(0, 0) = (abs(ftReadings[3]) < threshold ? 0 : ftReadings[3] * coefTorques);
    torques.at<double>(0, 1) = (abs(ftReadings[4]) < threshold ? 0 : ftReadings[4] * coefTorques);
    torques.at<double>(0, 2) = (abs(ftReadings[5]) < threshold ? 0 : ftReadings[5] * coefTorques * 5);
