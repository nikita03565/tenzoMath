...
{
    std::stringstream tmpBuf;
    tmpBuf << createStringToSend(xToRobot, yToRobot, zToRobot, wToRobot, pToRobot, rToRobot) << ' ' << seg << ' ' <<
            type
            << " 0";
    return _strdup(tmpBuf.str().c_str());
}
...
void FanucM20iA::checkCoordsLimits(const double& x, const double& y, const double& z) const
{
    ...
    {
        if (x > _maxValueOf.x())
            errorMessage = "Exceeding the maximum limits along the X axis";
        if (x < _minValueOf.x())
            errorMessage = "Exceeding the minimum limits along the X axis";

        if (y > _maxValueOf.y())
            errorMessage = "Exceeding the maximum limits along the Y axis";
        if (y < _minValueOf.y())
            errorMessage = "Exceeding the minimum limits along the Y axis";

        if (z > _maxValueOf.z())
            errorMessage = "Exceeding the maximum limits along the Z axis";
        if (z < _minValueOf.z())
            errorMessage = "Exceeding the minimum limits along the Z axis";
    }
